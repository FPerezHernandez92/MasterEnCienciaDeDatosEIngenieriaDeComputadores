...
run_metodo_fold <- function(i, x, metodo = "knn", tt="test"){
  ...
  aplicar_metodo_fold(x_tra[,-15],train_labels,test,test_labels,metodo)
}
...
cor(australian[,-15])
##Podemos ver como las variables V9 y V10 estan correladas
cor(australian[,-15])[9,10]
...
##Ahora si aplico estas funciones a los conjuntos de train y test facilitados podemos tener:
##Primero creo las funciones para hacer todo el proceso de forma automática
aplicar_metodo_fold <- function(train,train_labels,test,test_labels,metodo="knn"){
  if (metodo == "knn"){
    knnFit <- train(train, train_labels, method="knn", metric="Accuracy", tuneGrid = data.frame(.k=1:15))
    knnPred <- predict(knnFit, newdata = test)
...
##Como V10 y V9 vimos que estaban correladas por lo que voy a juntarlas
head(australian)
summary(australian)
...
##Por lo tanto voy a probar a unirlas
run_metodo_fold <- function(i, x,metodo = "knn", tt="test",modelo=0){
  file <- paste(x, "-10-", i, "tra.dat", sep="")
  x_tra <- read.csv(file, comment.char="@")
  In <- length(names(x_tra)) - 1 
...
##vamos a eliminar algunas de las que su p-value es mayor a 0.5 como V1 y V3
...
##Donde veo como V7, que es la que tiene un p-value mayor, no guarda correlación con ninguna, por lo que también la eliminaré
...
##Por lo que voy a probar un último modelo con ambas variables unidas V8*V9*V10
